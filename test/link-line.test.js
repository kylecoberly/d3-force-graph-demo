...
test("linkLine works left to right", () => {
	const source = {
		x: 0,
		y: 0,
	}
	...
	expect(x1).toBeGreaterThan(source.x)
	expect(x1).toBeLessThan(x2)
	expect(x2).toBeLessThan(target.x)
...
test.skip("linkLine works right to left", () => {
	...
	expect(x1).toBeLessThan(source.x)
	expect(x1).toBeGreaterThan(x2)
	expect(x2).toBeGreaterThan(target.x)
